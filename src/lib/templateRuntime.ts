...
export function normalizeTemplates(templates: Partial<Template>[] = []): Template[] {
  return templates.map((template, order) => ({
    ...
    folder: typeof template.folder === 'string' && template.folder.trim() ? template.folder.trim() : null,
    ...
    deletedAt: template.deletedAt ? String(template.deletedAt) : null,
  ...
  })).filter((template) => !template.deletedAt).sort((a, b) => a.order - b.order)
}
